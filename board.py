...
class PlayerBoard:
    ...
    def _build_regions(self) -> List[Region]:
        ...
        for coord, slot in self.hex_map.grid.items():
            ...
            blob_coords = self.hex_map.flood_fill_region_from(coord)
    ...
    def can_place_tile_at(self, tile: Tile, coord: Tuple[int, int]) -> bool:
        ...
        - slot pas occupé
        - type compatible
        (Ici on ne vérifie PAS les règles globales genre 'doit
        être adjacent à une tuile déjà posée',
        mais tu pourras l’ajouter.)
        """
    ...
    def get_all_completed_regions(self) -> List[Region]:
        """Utile en fin de partie pour scorer les bonus qui dépendent des régions complètes."""
    ...
    def debug_print_board(self) -> None:
        ...
        for coord, slot in sorted(self.hex_map.grid.items()):
            print(coord, slot.allowed_type, "OCC" if slot.is_occupied else "free")
